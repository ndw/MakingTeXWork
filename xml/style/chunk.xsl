...
<xsl:template match="bookinfo/copyright" mode="titlepage.mode">
  <p class="copyright">
    <a href="{concat('mtwcpyright',$html.ext)}">Copyright</a>
    <xsl:text> &#xA9; </xsl:text>
  ...
  <xsl:call-template name="write.chunk">
    <xsl:with-param name="filename">
      <xsl:call-template name="make-relative-filename">
        ...
        <xsl:with-param name="base.name" select="concat('mtwcpyright',$html.ext)"/>
      </xsl:call-template>
    </xsl:with-param>
    <xsl:with-param name="content">
      <xsl:apply-templates select="/book" mode="mtwcpyright-mode"/>
    </xsl:with-param>
  </xsl:call-template>
</xsl:template>

<xsl:template match="book" mode="mtwcpyright-mode">
  <html>
